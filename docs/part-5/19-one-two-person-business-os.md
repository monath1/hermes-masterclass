...
Hermes may draft an SOP from observed work, compare versions, identify missing checks, and run synthetic tests. The process owner approves it. Automate only the stable Green segment after representative manual runs. Hooks observe lifecycle events and can enforce a narrow callback, but third-party hooks run in-process. Prefer declarative hooks, pin reviewed code, and choose fail-closed behavior for a load-bearing check. A hook is not an accountant, manager, or policy owner.
